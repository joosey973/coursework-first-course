...
class TopLevel(BaseMNK, ctk.CTkToplevel):
    ...
    def create_function_fields(self):
        approximation_type_label = ctk.CTkLabel(self, text='Выберите тип аппроксимации', font=('Arial', 15, 'bold'))
        approximation_type_label.place(relx=0.3, rely=0.05)
        
        self.approximation_type_var = tk.StringVar(value='Полиномиальная регрессия')
        self.approximation_type_menu = ctk.CTkOptionMenu(
            self, 
            values=['Полиномиальная регрессия', 'Экспоненциальная регрессия', 'Логарифмическая регрессия', 'Дробно-рациональная (общая)'],
        ...
        approxima_view_label = ctk.CTkLabel(self, text='Введите вид аппроксимируемой функции', font=('Arial', 15, 'bold'))
        self.approxima_view_field = ctk.CTkEntry(self, width=340, height=33, corner_radius=15)
        
        approxima_view_label.place(relx=0.3, rely=0.2)
        self.approxima_view_field.place(relx=0.3, rely=0.27)
        
        solve_btn = ctk.CTkButton(self, text='Найти аппроксимируемую функцию', corner_radius=15, command=self.find_approxima_func)
        solve_btn.place(relx=0.34, rely=0.35)

        approxima_func_label = ctk.CTkLabel(self, text='Аппроксимируемая функция', font=('Arial', 15, 'bold'))
        self.approxima_func_field = ctk.CTkEntry(self, width=340, height=33, corner_radius=15)
        ...
        try:
            ...
            for i in range(n_params):
                for j in range(n_params):
                    G[i][j] = sum([basis_num[i](x) * basis_num[j](x) for x in x_data])

            G_2 = np.array(G)
            if np.linalg.cond(G_2) > 1e10:
                self.show_popup('Ошибка: вырожденная система уравнений!\nПроверьте, что параметры функции линейно независимы.\nНапример, запись "a*x^2 + b*x^2" приводит к зависимости параметров.', 'error')
                return None, None

            B = [[0] for i in range(n_params)]
            for i in range(n_params):
                B[i][0] = sum([y * basis_num[i](x) for x, y in zip(x_data, y_data)])
            coefficients = self.matrix_multiplication(self.find_reversed_matrix(G), B)
            coefficients = [float(coef[0]) for coef in coefficients]
            return coefficients, params
        except Exception as e:
            self.show_popup(f'Ошибка в синтаксисе функции: {func_str}\nПроверьте правильность записи.', 'error')
            self.on_change_clear()
            return None, None
    ...
    def universal_rational_approximation(self, func_str):
        try:
            ...
            for x_val, y_val in zip(x_data, y_data):
                ...
                free_term = -float(linear_expr.subs({param_syms[p]: 0 for p in params}))
            ...
            first_nonzero = np.argmax(np.abs(coefficients) > 1e-6) if len(coefficients) > 0 else 0
    ...
    def check_linear_independence(self, basis_functions, x_data):
        ...
        if rank < len(basis_functions):
            self.show_popup(
                ...
                f'Ранг матрицы: {rank}, количество параметров: {len(basis_functions)}\n'
    ...
    def check_enough_points(self, required_points=2):
        if len(self.x_y_list) < required_points:
            self.show_popup(f'Недостаточно точек для аппроксимации! Требуется минимум {required_points} точек, получено {len(self.x_y_list)}', 'error')
            return False
        return True

    def check_enough_points_for_exponential(self):
        if len(self.x_y_list) < 2:
            self.show_popup(f'Недостаточно точек для экспоненциальной аппроксимации! Требуется минимум 2 точки, получено {len(self.x_y_list)}', 'error')
            return False
        
        invalid_points = [(x, y) for x, y in self.x_y_list if y <= 0]
        if invalid_points:
            self.show_popup(f'Для экспоненциальной регрессии все y должны быть > 0! Найдены точки с y <= 0: {invalid_points[:3]}...', 'error')
            return False
        return True

    def check_enough_points_for_logarithmic(self):
        if len(self.x_y_list) < 2:
            self.show_popup(f'Недостаточно точек для логарифмической аппроксимации! Требуется минимум 2 точки, получено {len(self.x_y_list)}', 'error')
            return False
        
        invalid_points = [(x, y) for x, y in self.x_y_list if x <= 0]
        if invalid_points:
            self.show_popup(f'Для логарифмической регрессии все x должны быть > 0! Найдены точки с x <= 0: {invalid_points[:3]}...', 'error')
            return False
        return True

    def check_enough_points_for_rational(self, func_str):
        if len(self.x_y_list) < 2:
            self.show_popup(f'Недостаточно точек для дробно-рациональной аппроксимации! Требуется минимум 2 точки, получено {len(self.x_y_list)}', 'error')
            return False
        ...
        if len(self.x_y_list) < n_params:
            self.show_popup(f'Недостаточно точек для определения {n_params} параметров! Требуется минимум {n_params} точки, получено {len(self.x_y_list)}', 'error')
            return False
        
        if len(self.x_y_list) < n_params + 1:
            self.show_popup(f'Для устойчивости дробно-рациональной аппроксимации рекомендуется иметь хотя бы на 1 точку больше, чем параметров. Сейчас: {len(self.x_y_list)} точек, параметров: {n_params}', 'warning')
        
        x_values = [x for x, y in self.x_y_list]
        if len(x_values) != len(set(x_values)):
            self.show_popup('Обнаружены повторяющиеся значения x! Это может привести к вырожденности системы', 'warning')
        
        return True

    def check_enough_points_for_polynomial(self, func_str):
        if len(self.x_y_list) < 2:
            self.show_popup(f'Недостаточно точек для полиномиальной аппроксимации! Требуется минимум 2 точки, получено {len(self.x_y_list)}', 'error')
            return False
        ...
        if approximation_type == 'Полиномиальная регрессия':
            ...
            if coefficients is not None:
                ...
                for param, coef in zip(params, coefficients):
                    func_str_result = func_str_result.replace(param, f'{coef:.6f}')
                func_str_result = func_str_result.replace('+ -', '-').replace('- -', '+').replace('+-', '-').replace('--', '+')
                self.approxima_func_field.configure(state='normal')
                self.approxima_func_field.delete(0, 'end')
            ...
            coefficients, params = self.universal_rational_approximation(func_str)
    ...
    def plot_approximation(self, func_str, func_type='polynomial'):
        ...
        self.ax.scatter(x_data, y_data, color='red', label='Исходные точки', zorder=5)
        ...
        try:
            if func_type == 'exponential':
                ...
                func_num = sp.lambdify(x_sym, func, modules=['numpy', {'exp': np.exp, 'log': np.log, 'ln': np.log}])
            ...
            self.ax.plot(x_plot, y_plot, 'b-', label='Аппроксимация', linewidth=2)
            self.ax.text(0.5, 1.04, text_str,
    ...
    def parse_function(self, func_str):
        ...
        params = list(set([p for p in params if p not in ['x', 'exp', 'sin', 'cos', 'tan', 'log', 'sqrt', 'ln']]))
        params.sort()
        return params
