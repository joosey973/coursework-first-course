...
class Equation(BaseWindow):
    def __init__(self, parent, title, x=None, y=None):
        self.photos = []
        self.sections = []
        ...
        self.figure = None
        self.canvas = None
        super().__init__(parent, title, width=1200, height=700, x=x, y=y)
    ...
    def find_derivative(self, x, function_choice):
        return (
            self.derivative_of_first_function(x)
            if function_choice == 1
            else self.derivative_of_second_function(x)
        )

    def find_second_derivative(self, x, function_choice):
        return (
            self.second_derivative_of_first_function(x)
            if function_choice == 1
            else self.second_derivative_of_second_function(x)
        )
    ...
    def create_methods_fields(self):
        ...
        readonly_fields = [
            self.dihotomia_field,
            self.dihotomia_iter_field,
            self.hord_field,
            self.hord_iter_field,
            self.kas_field,
            self.kas_iter_field,
            self.comb_field,
            self.comb_iter_field,
            self.iter_field,
            self.iter_iter_field,
        ]
    ...
    def find_section(self, function_choice, a=0, b=0, step=0.05):
        ...
        while flag:
            ...
            ls = list(
                zip(function_values, first_derivative, second_derivative, x),
            )
            sections = []
            for i in range(1, len(ls)):
                if (
                    ls[i][0] * ls[i - 1][0] < 0
                    and ls[i][1] * ls[i - 1][1] > 0
                    and ls[i][2] * ls[i - 1][2] > 0
                ):
                    sections.append(
                        (np.round(ls[i - 1][-1], 5), np.round(ls[i][-1], 5)),
                    )
                    flag = False
    ...
    def kas(self, section, eps, function_choice):
        ...
        while flag:
            if function_choice == 1:
                xn = xn - self.first_function(
                    xn,
                ) / self.derivative_of_first_function(xn)
            else:
                xn = xn - self.second_function(
                    xn,
                ) / self.derivative_of_second_function(xn)
    ...
    def hord(self, section, eps, function_choice):
        ...
        while flag:
            if function_choice == 1:
                bn = bn - (
                    self.first_function(bn)
                    * (bn - a)
                    / (self.first_function(bn) - self.first_function(a))
                )
            else:
                bn = bn - (
                    self.second_function(bn)
                    * (bn - a)
                    / (self.second_function(bn) - self.second_function(a))
                )
    ...
    def comb(self, section, eps, function_choice):
        ...
        while flag:
            if function_choice == 1:
                an = an - self.first_function(
                    an,
                ) / self.derivative_of_first_function(an)
                bn = bn - (
                    self.first_function(bn)
                    * (bn - a)
                    / (self.first_function(bn) - self.first_function(a))
                ...
                an = an - self.second_function(
                    an,
                ) / self.derivative_of_second_function(an)
                bn = bn - (
                    self.second_function(bn)
                    * (bn - a)
                    / (self.second_function(bn) - self.second_function(a))
    ...
    def iterat(self, section, eps, function_choice):
        ...
        f1a, f1b = self.find_derivative(
            temp_a,
            function_choice,
        ), self.find_derivative(temp_b, function_choice)
