...
class MKR(BaseWindow):
    ...
    def __init__(self, parent, title, width=1300, height=700):
        super().__init__(parent, title, width, height)
    
    def create_task_parametrs_fields(self):
        frame = ctk.CTkFrame(self, corner_radius=15, width=450, height=250)
        frame.place(relx=0.03, rely=0.05)
        frame_label = ctk.CTkLabel(frame, text='Задайте параметры', corner_radius=15, fg_color="#d6d1d1", width=30, height=30, text_color='#2b2b2b')
        frame_label.place(relx=0.02, rely=0.02)

        self.p_x_field = ctk.CTkEntry(frame, corner_radius=15, width=100, height=30)
        ctk.CTkLabel(frame, text='p(x):').place(relx=0.03, rely=0.17)
        self.p_x_field.place(relx=0.22, rely=0.17)
        self.q_x_field = ctk.CTkEntry(frame, corner_radius=15, width=100, height=30)
        ctk.CTkLabel(frame, text='q(x):').place(relx=0.03, rely=0.37)
        self.q_x_field.place(relx=0.22, rely=0.37)
        self.r_x_field = ctk.CTkEntry(frame, corner_radius=15, width=100, height=30)
        ctk.CTkLabel(frame, text='r(x):').place(relx=0.03, rely=0.57)
        self.r_x_field.place(relx=0.22, rely=0.57)
        self.n_field = ctk.CTkEntry(frame, corner_radius=15, width=100, height=30)
        ctk.CTkLabel(frame, text='n (число отрезков):', wraplength=80, justify=ctk.LEFT).place(relx=0.03, rely=0.77)
        self.n_field.place(relx=0.22, rely=0.77)
        
        self.x_0_field = ctk.CTkEntry(frame, corner_radius=15, width=100, height=30)
        ctk.CTkLabel(frame, text='x0:').place(relx=0.55, rely=0.17)
        self.x_0_field.place(relx=0.64, rely=0.17)
        self.x_k_field = ctk.CTkEntry(frame, corner_radius=15, width=100, height=30)
        ctk.CTkLabel(frame, text='xk:').place(relx=0.55, rely=0.37)
        self.x_k_field.place(relx=0.64, rely=0.37)
        self.y_0_field = ctk.CTkEntry(frame, corner_radius=15, width=100, height=30)
        ctk.CTkLabel(frame, text='y(x0):').place(relx=0.55, rely=0.57)
        self.y_0_field.place(relx=0.64, rely=0.57)
        self.y_k_field = ctk.CTkEntry(frame, corner_radius=15, width=100, height=30)
        ctk.CTkLabel(frame, text='y(xk):').place(relx=0.55, rely=0.77)
        self.y_k_field.place(relx=0.64, rely=0.77)
    
    def create_buttons_with_method(self):
        frame = ctk.CTkFrame(self, corner_radius=15, width=450, height=250)
        frame.place(relx=0.5, rely=0.05)
        ctk.CTkLabel(frame, text='Метод решения', corner_radius=15, fg_color="#d6d1d1", width=30, height=30, text_color='#2b2b2b').place(relx=0.02, rely=0.02)

        self.radio_var = tk.IntVar(value=0)
        vr_1 = ctk.CTkRadioButton(frame, text='Прогонка', variable=self.radio_var, value=0)
        vr_2 = ctk.CTkRadioButton(frame, text='Матричный', variable=self.radio_var, value=1)
        vr_3 = ctk.CTkRadioButton(frame, text='Крамер', variable=self.radio_var, value=2)

        vr_1.place(relx=0.05, rely=0.25)
        vr_2.place(relx=0.05, rely=0.5)
        vr_3.place(relx=0.05, rely=0.75)

        solve_btn = ctk.CTkButton(frame, text='Решить', width=200, corner_radius=15, command=self.solve)
        insrt_btn = ctk.CTkButton(frame, text='Вставить по умолчанию', width=200, corner_radius=15, command=self.insert_base_vals)
        ctk.CTkButton(frame, text='Вернуться в меню<', width=200, corner_radius=15, command=lambda: self.create_new_window('Меню')).place(relx=0.5, rely=0.75)
    ...
    def solve_by_kramer(self, A, b):
        ...
        if abs(det_A) < 1e-10:
            self.show_popup("Определитель равен нулю, метод Крамера неприменим")
            return None
    ...
    def solve(self):
        ...
        for field_name, (field_widget, display_name, expected_type) in fields.items():
            ...
            if expected_type is str:
                ...
                if field_name == 'n' and not raw_value.isdigit():
                    self.show_popup(f'Значение поля "{display_name}" должно быть int')
                    return
                ...
                if expected_type is int:
                    ...
                    try:
                        ...
                        self.show_popup(f'Значение поля "{display_name}" должно быть float/int')
                        return
        ...
        if ('log' in self.p or 'log' in self.q or 'log' in self.r or 'ln' in self.p or 'ln' in self.q or 'ln' in self.r) and not all([val > 0 for val in x]):
    ...
    def update_polinom_and_gprahic(self, coefs, x, y):
        ...
        if x and y:
            ...
            self.ax.plot(x_dots, y_dots, 'b-', linewidth=2, label='Интерполяционный полином')
            self.ax.scatter(x, y, label='Узлы интерполяции', marker='o', color='red')
            self.ax.legend()
            self.canvas.draw()
        else:
            self.ax.clear()
    ...
    def matrix_method(self, x, h):
        ...
        result_matrix = self.matrix_multiplication(reversed_matrix, [[i] for i in coefs[-1]])
    ...
    def progonka_method(self, x, h):
        ...
        if np.isnan(alpha[0]) or np.isinf(alpha[0]) or np.isnan(beta[0]) or np.isinf(beta[0]):
        ...
        for i in range(1, self.n + 1):
            ...
            if abs(denominator) < 1e-15:
                self.show_popup(f'Деление на ноль в методе прогонки на шаге {i}')
                return
            
            alpha[i] = B[i] / denominator
            beta[i] = (beta[i - 1] * A[i] - F[i]) / denominator
            if np.isnan(alpha[i]) or np.isinf(alpha[i]) or np.isnan(beta[i]) or np.isinf(beta[i]):
                self.show_popup(f'Некорректные значения в методе прогонки на шаге {i}')
                return
        ...
        for i in range(self.n - 1, -1, -1):
            ...
            if np.isnan(y[i]) or np.isinf(y[i]):
                self.show_popup(f'Некорректное значение y[{i}] в методе прогонки')
                return
    ...
    def show_polinom(self):
        ctk.CTkLabel(self, text='Формула полинома', corner_radius=15, fg_color="#d6d1d1", width=30, height=30, text_color='#2b2b2b').place(relx=0.4, rely=0.85)
        self.polinom_field = ctk.CTkEntry(self, width=720, corner_radius=15)
    ...
    def show_table(self, x_values=[], y_values=[]):
        ...
        headers_frame = ctk.CTkFrame(scroll_frame, fg_color='#d6d1d1', corner_radius=8)
    ...
    def show_graphic(self):
        graphic_frame = ctk.CTkFrame(self, corner_radius=15, width=450, height=350)
        graphic_frame.place(relx=0.03, rely=0.45)

        title_label = ctk.CTkLabel(graphic_frame, text='МКР', font=('Arial', 15, 'bold'))
        ...
        self.canvas.draw()
        self.canvas.get_tk_widget().place(relx=0.5, rely=0.54, anchor=tk.CENTER, width=400, height=300)
